                                ...
                                <div class="tab-content">
                                    <div id="semua" class="tab-pane active">
                                        ...
                                        if (isset($_GET['judul'])) {
                                            ...
                                            if ($pengajuan == "") {
                                                $query_semua = "SELECT id, program, periode, dosen_wali, nama, nim, ipk, keterangan, total_sks, pembimbing, jadwal, jenis, lokasi, status 
                                     FROM pendaftaran 
                                     WHERE pembimbing = '$nama_depan' 
                                     AND (tanggal BETWEEN '$awal' AND '$akhir') 
                                     AND status LIKE '%$status%' 
                                     ORDER BY id DESC";
                                            } else {
                                                $query_semua = "SELECT id, program, periode, dosen_wali, nama, nim, ipk, keterangan, total_sks, pembimbing, jadwal, jenis, lokasi, nilai, status 
                                     FROM pendaftaran 
                                     WHERE pembimbing = '$nama_depan' 
                                     ...
                                     ORDER BY id DESC";
                                            ...
                                            $query_semua = "
                    SELECT p.id, p.program, p.periode, p.dosen_wali, p.nama, p.nim, p.ipk, p.keterangan, p.total_sks, p.pembimbing, p.jadwal, p.jenis, p.lokasi, p.nilai, p.status 
                    FROM pendaftaran AS p
                    WHERE p.pembimbing = '$nama_depan' 
                        ...
                        <table class="table table-hover table-striped table-bordered">
                            ...
                            <tbody>';
                                            ...
                                            while ($data_semua = mysqli_fetch_assoc($result_semua)) {
                                                echo '<tr>
                            <td>' . $no_semua . '</td>
                            <td>' . htmlspecialchars($data_semua['periode'], ENT_QUOTES) . '</td>
                            <td>' . htmlspecialchars($data_semua['nama'], ENT_QUOTES) . '</td>
                            <td>' . htmlspecialchars($data_semua['nim'], ENT_QUOTES) . '</td>
                            <td>' . htmlspecialchars($data_semua['dosen_wali'], ENT_QUOTES) . '</td>
                            <td>' . htmlspecialchars($data_semua['pembimbing'], ENT_QUOTES) . '</td>
                            <td>' . htmlspecialchars($data_semua['jadwal'], ENT_QUOTES) . '</td>
                            <td>' . htmlspecialchars($data_semua['jenis'], ENT_QUOTES) . '</td>
                            <td>' . htmlspecialchars($data_semua['lokasi'], ENT_QUOTES) . '</td>
                            <td>' . htmlspecialchars($data_semua['nilai'], ENT_QUOTES) . '</td>
                            <td>
                                <a href="edit_presentasi?id=' . $data_semua['id'] . '">
